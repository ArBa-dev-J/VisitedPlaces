...
export const newVisitedPlaceM = async (newPlace) => {
    const { name, place_type, description, filename, fileHash, address, rating, is_free, city_id } = newPlace;

    const isFreeBool = is_free == "true";

    const upload = await sql`
    INSERT INTO places (name, place_type, description, image_url, fileHash, address, rating, is_free, city_id)
